...
function SAXParser (strict) {
  ...
  function write (s) {
    ...
    else if (closed) throw new Error("Cannot write after close.");
    else buffer += s;
    if (!going) defer(playTheSax);
    going = true;
    return this;
  };
  ...
  function closeText () {
    if (textNode) self.emit("text", textNode);
    textNode = "";
  };
  ...
  function startTagDone () {
    makeTag();
    if (!strict && selfClosing.indexOf(tag.name) !== -1) {
      tagSelfClose();
      continue;
    }
    if (!tag) throw "oops";
...
SAXParser.prototype = { __proto__ : events.EventEmitter.prototype };

})(
  typeof exports === "object" ? exports : (this.sax = {}),
  typeof process === "object" && process.nextTick || function (f) {
    setTimeout(f, 0)
  },
  typeof require === "function" && require("events") || {
    EventEmitter : { prototype : {
      ...
      emit : function (ev, data) {
        for (var i = 0, l = this._listeners[ev].length; i < l; i ++) {
          this._listeners[ev][i](data);
        }
      }
    }}}
);