...
sax.createStream = function () {
  ...
  parser.onopentag = function (node) {
    stream.emit('tag', node)
  };
  parser.onattribute = function (attr) {
    stream.emit('attribute', attr)
  };
  ...
  stream.__defineSetter__('onattribute', function (val) {
    parser.onattribute = function (attr) {
      stream.emit('attribute', attr)
      val(attr)
    }
  })
  stream.__defineSetter__('onopentag', function (val) {
    parser.onopentag = function (node) {
      stream.emit('tag', node)
      val(node)
    }
  })
  stream.__defineSetter__('ontext', function (val) {
    parser.onend = function (text) {
      stream.emit('text', text)
      val(text)
    }
  }) 
  stream.__defineSetter__('onerror', function (val) {
    parser.onerror = function (e) {
      stream.emit('error', e)
      val(e)
    }
