    ...
    if (len > maxAllowed) {
      ...
      switch (buffers[i]) {
        ...
        case "cdata":
          ...
          break
        default:
          console.error(buffers[i], parser[buffers[i]], maxAllowed)
...
, SCRIPT_BODY               : S++ // <script> var foo = 1;
, SCRIPT_QUOTED             : S++ // <script> var foo = "one";
, SCRIPT_LT                 : S++ // 1 < 2
, OPEN_TAG                  : S++ // <strong
...
function openTag (parser, selfClosing) {
  ...
  var tagname = parser.tag.name.toUpperCase();
  if (!selfClosing) {
    parser.tag = null;
    parser.tagName = "";
    if (tagname === 'SCRIPT' || tagname === 'STYLE') {
        parser.maybeScriptCloseTag = "";
        parser.state = S.SCRIPT_BODY;
    }
...
function write (chunk) {
  ...
  while (parser.c = c = chunk.charAt(i++)) {
    ...
    switch (parser.state) {
      ...
      case S.SCRIPT_BODY:
        if (c === "<") {
          parser.state = S.SCRIPT_LT;
        } else if (is(quote, c)) {
          parser.state = S.SCRIPT_QUOTED;
          parser.q = c;
        }
      continue;
      case S.SCRIPT_LT:
        parser.maybeScriptCloseTag += c;
        if (parser.maybeScriptCloseTag === '/script' ||
            parser.maybeScriptCloseTag === '/style') {
            parser.tagName = parser.maybeScriptCloseTag.slice(1);
            parser.maybeScriptCloseTag = "";
            parser.state = S.CLOSE_TAG;
        }
        else if (parser.maybeScriptCloseTag.length > 7) {
            parser.maybeScriptCloseTag = "";
            parser.state = S.SCRIPT_BODY;
        }
      continue;
      case S.SCRIPT_QUOTED:
        if (c === parser.q) {
          parser.state = S.SCRIPT_BODY;
          parser.q = "";
        }
