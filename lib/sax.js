...
var whitespace = "\r\n\t ",
  ...
  selfClosing = [
    ...
    "FRAME","COL","PARAM" // know of any more?
  ];
      ...
      switch (state) {
        ...
        case S_OPEN_WAKA:
          ...
          if (c === "!") {
            ...
            strictFail(
              "Unencoded < not at start of tag, comment, or processing instruction");
            textNode += "<" + c;
            state = S_TEXT_NODE;
          }
        continue;
        ...
        case S_COMMENT:
          ...
          } else state = S_COMMENT_EXCL_DASH;
        continue;
        case S_DOCTYPE:
          if (sawRoot) {
            strictFail("Doctype found after root node.");
            comment = doctype + c;
            state = S_COMMENT_BODY;
          }
          // note: doctype must be, at minimum <!doctype something>,
          // and must come before the root node, and must occur once.
          if (is(whitespace, c)) {
            if (doctype.toUpperCase() === DOCTYPE) {
              doctypeBody = "";
              state = S_DOCTYPE_BODY;
            } else {
              strictFail("Invalid doctype");
              comment = doctype+c;
              state = S_COMMENT_BODY;
            }
          } else {
            // still reading
            doctype += c;
            if (DOCTYPE.indexOf(doctype.toUpperCase()) !== 0) {
              strictFail("Invalid doctype");
              comment = doctype;
              doctype = "";
              state = S_COMMENT_BODY;
        ...
        case S_DOCTYPE_BODY:
          if (is(quote, c)) {
            q = c;
            state = S_DOCTYPE_BODY_QUOTED;
            doctypeBody += c;
          } else if (c === ">") {
            emitNode("doctype", doctypeBody)
            state = S_TEXT_NODE;
          } else doctypeBody += c;
        continue;
        case S_DOCTYPE_BODY_QUOTED:
          doctypeBody += c;
          if (c === q) state = S_DOCTYPE_BODY;
        continue;
