...
function closeValue (parser) {
  ...
  if (parser.textNode) emit(parser, "onvalue", parser.textNode);
  parser.textNode = "";
}
...
function end (parser) {
  console.log(saj.STATE[parser.state],"");
...
function write (chunk) {
  ...
  while (parser.c = c = chunk.charAt(i++)) {
    console.log(i,c,saj.STATE[parser.state]);
    parser.position ++;
    if (c === "\n") {
      parser.line ++;
      parser.column = 0;
    ...
    switch (parser.state) {
      ...
      case S.CLOSE_OPEN_OBJECT:
        if (is(whitespace, c)) continue;
        emitValue(parser, 'onopenobject', buffer.string);
        buffers.string = '';
        parser.state  = S.VALUE;
      continue;
      ...
      case S.STRING:
        ...
        if(c === '"') {
          ...
          while(c && !(c === ":" || c === ',' || c === ']' || c === '}')) {
            ...
            console.log(i,c,saj.STATE[parser.state], 'derp');
            c = chunk.charAt(i++);
          }
          if (c === ':') parser.state = S.CLOSE_OPEN_OBJECT;
            ...
            else if (c === ']') parser.state = S.CLOSE_ARRAY;
            else if (c === '}') parser.state = S.CLOSE_OBJECT;
